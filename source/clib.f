...
c                                 check that solvus tolerance is not too small:
      if (nopt(8).gt.0d0.and.isoct.gt.0) then 
         ...
         if (icopt.eq.1.and.nopt(13)/nopt(19).ge.nopt(8)) then
c                                 non-adaptive minimization Schreinemakers diagram
            nopt(8) = 1.2d0 * nopt(13)/nopt(19)
            write (*,1080) nopt(8)

         else if (icopt.le.3.and.nopt(13)/nopt(18).ge.nopt(8))
     *                                                           then 
c                                 other non-adaptive minimization calculations
            nopt(8) = 1.2d0 * nopt(13)/nopt(18)
            write (*,1080) nopt(8) 
...
c                                 default search increment
      ...
      subroutine input1 (first,output,n4name)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     *          n3name,n4name,n9name,y*1,sname*10,opname
...
c                                 use error condition to determine which:
      read (n1,1000) opname
      read (opname,*,iostat=ierr) icopt 

      if (ierr.eq.0) then 
c                                 if no error, old version
         opname = 'perplex_option.dat'
      else
c                                 new version, filter opname and read icopt
         call enblnk (opname)
