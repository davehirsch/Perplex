...
c psopen - subroutine to open LUN nps, write prologue
      ...
      character fname*100, record(100)*1
...
c----------------------------------------------------------------------
      ...
      read (fname,1000) (record(i), i=1,80)
 
      call pschct (nchar,100,record)
 
      if (nchar.gt.97) nchar = 97
 
      record(nchar) = '.'
      record(nchar+1) = 'p'
      record(nchar+2) = 's'
 
      write (fname,1000) (record(i), i = 1, nchar + 2)
 
      open (nps,file=fname,status='unknown')
c                                 output eps prolog
      call psprol (nps)       
