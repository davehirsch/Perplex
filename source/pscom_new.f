...
c psylbl - subroutine to put on y-axis labels.
     ...
     *             spline,half,tenth,grid,fill,label
      ...
      do i = 1, iy1
         ...
         if (x.lt.tmin) tmin = x
         call pstext (x, y + ydc, numbs(i), ic(i),mdAlgn,rtAlgn)
...
c psxlbl - subroutine to put on x-axis labels.
     ...
     *             spline,half,tenth,grid,fill,label
      ...
      do i = 1, ix1
         ...
         if (x.ne.xmin) then 
            t = x - dfloat(ic(i)) * xdc 
            call pstext (t, y, numbs(i), ic(i),tpAlgn,ctAlgn)
...
c psaxes - subroutine to output (sloppy) axes.
     ...
     *                 xtic2,ytic2,tmin,x,y,nscold

      integer jop0, i, nchar, iftold
...
c----------------------------------------------------------------------
      ...
      if (debug) print *, "IN PSAXES.  vnm=", vnm(1), vnm(2)
...
c                                       numeric axis labels:
      if (debug) print *, "IN PSAXES.  Writing y tick labels."
c  DMH Begin tracking min-X text coordinates:	  
      call trkMnX()      
c  Actuall write the tick labels:
      call psylbl (y0, dy, tmin) 
 
c DMH: bold face the axis labels and make them larger - note that a
c   bug in Cairo doesn't allow mixing bold and non-bold on Quartz
c   (Mac OSX), or something like that, which is why the bolding
c   is currently disabled
      iftold = ifont
      nscold = nscale
      ifont = 7
      nscale = nscale * 1.4d0
c                                       y-axis name
      ...
      if (debug) print *, "IN PSAXES.  Writing y axis label."
c DMH note that with the useMnX flag, the supplied x coordinate is ignored
      call pstext (x,y,vnm(2),0,btAlgn+1000,ctAlgn+useMnX)
      
c DMH restore original size and font
      ifont = iftold
      nscale = nscold



      if (debug) print *, "IN PSAXES.  Writing X tick labels."
      call pssctr (ifont,nscale,nscale,0d0)
      call psxlbl (x0, dx)
...
c                                       x-axis name
      ...
      if (debug) print *, "IN PSAXES.  Writing x axis label."
      call pstext (x,y,vnm(1),0,tpAlgn+100,ctAlgn)
...
c	DMH: ALTER THESE VALUEs (to zero) TO MAKE THEM NON-FUNCTIONAL
      dcx = 0d0 * xlen / 85d0 * cscale / xfac
      dcy = 0d0 * ylen / 85d0 * cscale
c                                     set axes scales
...
c----------------------------------------------------------------------
     ...
     *             spline,half,tenth,grid,fill,label
c---------------------------------------------------------------
      if (debug) PRINT *,'IN PSBLRB. nline=', nline

      x = 0
c DMH old      x = xmin - 1d1*dcx*nscale
      y = 0d0
c DMH old      y = ymax + 2.3d1*dcy*nscale
      ...
      do i = 1, nline
         ...
         call pssctr (ifont,curscl,curscl,0d0)
         call psublk (title(i),nchar)
         call pstext (x,y,title(i),nchar,mdAlgn+10,ltAlgn+10)
         y = y + 1d0
c DMH old         y = y - 2.4*dcy*nscale
      end do 
...
c----------------------------------------------------------------------
     ...
     *             spline,half,tenth,grid,fill,label
      ...
      do 
         ...
         call pstext (x,y,text,10,mdAlgn,ltAlgn)
...
c         else if (key.eq.'picture_transformation') then
c                                 picture transformation.   
c            read (strg1,*) dsx,dsy,dtx,dty,drot  
...
c                                 this is handled by graphic_output.   
c                                 ignore it quietly; leave else clause in here
c                                 to avoid unrecognized option warning
