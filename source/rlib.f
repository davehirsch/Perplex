...
c                                 normal polynomial:
     ...
     *               + (thermo(16,id) * p + thermo(14,id)) * p)
         ...
         if (lopt(4)) then 
c                                 helffrich-murnaghan EoS
            del = (thermo(11,id) + thermo(12,id)*t)*t +
     *          thermo(13,id)*dlog(t) + thermo(14,id)/t + 
     *          thermo(15,id)*dsqrt(t)
            rkt = thermo(16,id)*exp(-thermo(18,id)*del/thermo(3,id))

            del = (thermo(3,id) + del) * rkt/thermo(22,id) * (
     *          (1d0 + thermo(18,id)*p/rkt)**thermo(19,id) -
     *          (1d0 + thermo(20,id)  /rkt)**thermo(19,id))
...
c                              BM3
            if (b6.ne.0) then
c                              use expressions as in Holland & Powell for K, alpha 
               b1 = v*b1 
               b2 = v*b2/2d0 
               b3 = v*b3 
               b4 = -v*b4 
               b5 = 2d0*v*b5 
               v  = v - (b1-b2*tr)*tr - b3*dlog(tr) - b4/tr - 
     *              b5*dsqrt(tr)
            end if 
c                              if b6 = 0, assume ghirso expressions!
         else 
...
c                                 2nd strings
         ...
         read (text2,1000) (bitsy(i), i = nchar1, nchar2-1)  
         write (text,1000) (bitsy(i), i = 1, nchar2-1),
     *                     (' ',i = nchar2, nchar3)
      end if 
...
c-----------------------------------------------------------------------
      ...
      double precision turd2, trv, v0, k, turd, rat, rat2, c0, c1, c2, 
...
c                                 Schreinemakers use refine factor III
                     xnc(i,j) = xnc(i,j)/nopt(19)
                  else  
c                                 non-adaptive use refine factor II
                     xnc(i,j) = xnc(i,j)/nopt(18)
                  end if 
...
c                                 set ifp for melt endmembers
         if (ifp(id).le.0.and.(jsmod.eq.24.or.jsmod.eq.25)) 
     *       ifp(id) = -jsmod
c
         jend(im,2+i) = id
...
c                                 lord is the number of possible species
      if (lord.eq.1) then 
         ...
         do 
            ...
            if (tdp.lt.tol.and.gold-g.lt.1d2.or.tdp.eq.xtdp) exit
...
c-----------------------------------------------------------------------
     ...
     *        ist,i,j,iord,iterm,isite,isp,insp,kstot,
...
c--------------------------------------------------------------------------
     ...
     *                 ctot,vlaar,smix,esum,ctotal,omega,x
...
c                              y is the mole fraction of endmember l
         if (y(l).gt.0.9999d0.and.kdsol(l).gt.0) goto 999
...
c                                binary solutions
         if (pa(1).ge.0.04d0.and.pa(1).le.0.96d0) then
            write (names(iphct),1030) names(kdsol(knsp(1,im))),
     *                                idint(1d2*pa(1))
         else    
            write (names(iphct),1070) 
     *             names(kdsol(knsp(1,im))), 1d2*pa(1)             
         end if
...
c                                 is evaluated from speciation.
      ...
      else if (depend.or.nsite.ne.0) then 
