...
c                                 get input file 
      ...
      if (ier.ne.0) call error (67,1d0,iop0,fname)
...
1000  format (a)
1020  format (/,'Enter the Perple_X plot file name:')
...
c pschem - subroutine to output ternary chemographies.
      ...
      character names*8, title*162, record*72, yes*1, 
     *          sname*10, xname(k5)*8
 
      double precision x3(3),y3(3),xx(1000),yy(1000),style,x1,y1,y
...
c   DMH: moved data statements below other declarations to allow 
c      -fpedantic compilation
...
c                                  read header information 
     ...
     *   call error (65,xfac,iphct,'PSCHEM') 
      ...
      read (n4,1000) (vnm(i),i=1,ipot)
      read (n4,1000) title
c                                  names
      read (n4,1020) (names(i), i = 1, iphct)
c                                  phase coordinates
      read (n4,*) ((x(j,i),j = 1, 2), i=istct, iphct)
      read (n4,*) (ikp(i), i = 1, iphct)
c                                  solution names
      if (isoct.ne.0) read (n4,1090) (sname(i),i = 1, isoct)
c                                  write graphics code names for x variables:
      read (n4,1020) (xname(i), i = 1, icp)
c                                  -----------------------------------------------
         ...
         if (tlbl) then 
            do i = istct, iphct
               if (ivchk(i).ne.0) then
                  ...
                  x1 = x(1,i) + 15d-3
                  y1 = x(2,i) 
                  call pstext (x1,y1,names(i),8,mdAlgn,ltAlgn)
...
c                                       sectioning constraints
         ...
         do i = 1, ipot
            ...
            call pstext (0.75d0,y,record,nchar,mdAlgn,ltAlgn)
            y = y - 0.06d0*nscale
         end do 
 
         if (ifyn.eq.0) then 
            call pstext (0.75d0,y,'(fluid saturated)',17,mdAlgn,ltAlgn)
            y = y - 0.06d0*nscale
         end if 
...
99    end
      ...
      double precision xx(l5), yy(l5)
 
      integer iperm(2,3),idv(1000),idp(3),jdv(3),ias,i,ivert,j,id,
...
c subprogram to write a text label for a reaction
      ...
      character*8 names, text(400)*1, fname*10, string*400
...
40    continue
      ...
      write (string,1000) (text(i),i=1,400)
...
c psmixd - subroutine to draw binary mixed variable diagrams
      ...
      character*8 names, title*162, sname*10, string*400, y*1
      character*8 tname(5),xname(k5)
...
c                                 read solution names
      if (isoct.ne.0) read (n4,1050) (sname(i), i=1, isoct)
...
170         continue 
     ...
     *       ttext = tlast + 1.25d0*dcy*ascale
         call pstext (xmax+2d0*dcx,ttext,string,iend,mdAlgn,ltAlgn)
         tlast = ttext
...
c----------------------------------------------------------------------
      ...
      if (debug) print *, "IN PLINP.  Num phases=", iphct

      if (iphct.gt.0) then
         if (iphct.gt.k1) call error (65,c0,i,'PLINP')
         read (n4,1010) (names(i),i = 1, iphct)
         read (n4,*) (ikp(i), i = 1, iphct)
      end if

      if (debug) print *, "IN PLINP.  Phase1=", names(1)
      if (debug) print *, "IN PLINP.  Phase2=", names(2)

      if (isoct.ne.0) read (n4,1030) (sname(i), i = 1, isoct) 

      read (n4,1000) title
      read (n4,*) jvar,(jv(i),i=1,jvar),jpt
      read (n4,*) iind,idep,c0,c1,c2,c3,c4
      read (n4,*) (vmx(i),vmn(i),i=1,jvar)
      read (n4,1000) (vnm(i),i=1,jvar)

      if (debug) print *, "IN PLINP.  vmx=", vmx(1)
      if (debug) print *, "IN PLINP.  vmn=", vmn(2)
      if (debug) print *, "IN PLINP.  vnm1=", vnm(1)
      if (debug) print *, "IN PLINP.  vnm2=", vnm(2)
...
c                                   numeric label:
     ...
     *                      y(imid)+0.8d0*dcy*fac,lnms,6,mdAlgn,ltAlgn)
...
c                                  invariant point labels:
         ...
         call pstext (x(1)+.7d0*dcx,y(1)+.5d0*dcy,pnms,6,mdAlgn,ltAlgn)
...
c   icpd = the id of the compound if itis = 0
      ...
      character*10 sname, names*8, unnown
